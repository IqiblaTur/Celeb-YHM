...
def planning_page():
    ...
    with c3:
        report_minutes = st.number_input("STD öncesi görev hazır olma dk", min_value=60, max_value=240, value=150, step=15)
    with c4:
        debrief_minutes = st.number_input("STD sonrası kapanış dk", min_value=15, max_value=120, value=45, step=15)

    s1, s2, s3 = st.columns([0.25, 0.35, 0.40])
    with s1:
        service_min_people = st.number_input("Servis min kişi", min_value=1, max_value=10, value=4, step=1)
    with s2:
        align_shift_to_service = st.checkbox("Vardiya giriş/çıkışı servis saatine hizala", value=True)
    with s3:
        st.caption("Bu modda görev hazır olma saati korunur; personelin günlük giriş-çıkışı en uygun geliş/gidiş servisine göre yazılır.")
    ...
    if st.button("Haftalık Planı Oluştur", type="primary"):
        ...
        assignments_with_service, service_summary = add_service_plan(
            assignments,
            st.session_state.service,
            min_people=int(service_min_people),
            align_shift_to_service=bool(align_shift_to_service),
        )
        daily_schedule = build_daily_staff_schedule(assignments_with_service)
...
def show_plan_results():
    ...
    daily_schedule = st.session_state.get("daily_schedule", pd.DataFrame())
    ...
    with tab1:
        display_cols = [
            "date", "name", "route", "role", "airline", "out_flight",
            "arrival_service_time", "shift_start", "duty_start", "duty_end", "shift_end", "departure_service_time",
            "arrival_note", "departure_note", "rule_applied"
        ]
        display_cols = [c for c in display_cols if c in assignments.columns]
        view = assignments.sort_values([c for c in ["date", "shift_start_dt", "route", "out_flight", "slot_no"] if c in assignments.columns])
        st.data_editor(
            view[display_cols],
            width="stretch",
            num_rows="dynamic",
        ...
        st.caption("Shift start/end personelin servise göre yazılan giriş-çıkışıdır. Duty start/end ise uçuş için hazır olması gereken operasyon saatidir.")
    with tab2:
        if daily_schedule.empty:
            st.info("Günlük giriş-çıkış tablosu oluşturulamadı.")
        else:
            st.dataframe(daily_schedule, width="stretch", hide_index=True)
            st.caption("Bu sekme her personelin her gün kaçta gelip kaçta çıkacağını tek satırda gösterir.")
    with tab3:
    ...
    with tab5:
        ...
        hidden_cols = [
            "work_start_dt", "work_end_dt", "duty_start_dt", "duty_end_dt",
            "shift_start_dt", "shift_end_dt", "arrival_service_dt", "departure_service_dt", "name_key"
        ]
        excel_bytes = to_excel_bytes({
            "Shift Plan": assignments.drop(columns=[c for c in hidden_cols if c in assignments.columns]),
            "Daily Entry Exit": daily_schedule,
...
def service_page():
    ...
    if assignments.empty:
        ...
        return
    c1, c2, c3, c4 = st.columns(4)
    arrival_notes = assignments.get("arrival_note", pd.Series(dtype=str)).astype(str)
    with c1:
        st.metric("Servis OK geliş", int((arrival_notes == "Servis OK").sum()))
    with c2:
        st.metric("Servis önerisi", int(arrival_notes.str.contains("Servis önerisi", na=False).sum()))
    with c3:
        st.metric("Toplu taşıma geliş", int(arrival_notes.str.contains("Toplu Taşıma", na=False).sum()))
    with c4:
    ...
    st.markdown("#### Servis kontrol listesi")
    tt = assignments[(assignments["arrival_note"] != "Servis OK") | (assignments["departure_note"] != "Servis OK")]
    cols = ["date", "name", "route", "out_flight", "shift_start", "duty_start", "duty_end", "shift_end", "arrival_service_time", "arrival_note", "departure_service_time", "departure_note"]
    cols = [c for c in cols if c in tt.columns]
    st.dataframe(tt[cols], width="stretch", hide_index=True)
