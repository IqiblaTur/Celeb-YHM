...
def build_shift_plan(
    ...
    def candidate_score(person_row: pd.Series, flight_start: datetime) -> Tuple[int, int, str]:
        key = person_row["name_key"]
        day = flight_start.date()
        return (workload_minutes.get(key, 0), daily_task_count.get((key, day), 0), str(person_row["name"]))
...
def choose_service(work_start: datetime, work_end: datetime, route: str, arrival_times: List[str], departure_times: List[str]) -> Tuple[str, datetime, str, datetime]:
    # Last arrival service before work start; at least 30 min operational buffer preferred.
    arrival_target = work_start - timedelta(minutes=30)
    arr_options = [(t, service_datetime(work_start.date(), t, direction="arrival", target_dt=arrival_target)) for t in arrival_times]
    arr_options = [x for x in arr_options if x[1] <= arrival_target]
    arr_time, arr_dt = max(arr_options, key=lambda x: x[1]) if arr_options else ("Toplu Taşıma", work_start)

    # First departure service after work end.
    dep_target = work_end + timedelta(minutes=15)
    dep_options = [(t, service_datetime(work_end.date(), t, direction="departure", target_dt=dep_target)) for t in departure_times]
    dep_options = [x for x in dep_options if x[1] >= dep_target]
...
def add_service_plan(assignments: pd.DataFrame, service_df: Optional[pd.DataFrame] = None, min_people: int = 4) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, r in df.iterrows():
        ...
        start = r["work_start_dt"] if isinstance(r["work_start_dt"], datetime) else pd.to_datetime(r["work_start_dt"]).to_pydatetime()
        end = r["work_end_dt"] if isinstance(r["work_end_dt"], datetime) else pd.to_datetime(r["work_end_dt"]).to_pydatetime()
        arr_t, arr_dt, dep_t, dep_dt = choose_service(start, end, route, arrival_times, departure_times)
        rows.append({
    ...
    # Count service occupancy by route+time.
    arr_counts = df.groupby(["route", "arrival_service_dt"]).size().rename("arrival_service_count").reset_index()
    dep_counts = df.groupby(["route", "departure_service_dt"]).size().rename("departure_service_count").reset_index()
    df = df.merge(arr_counts, on=["route", "arrival_service_dt"], how="left")
    df = df.merge(dep_counts, on=["route", "departure_service_dt"], how="left")
    df["arrival_note"] = df["arrival_service_count"].apply(lambda n: "Servis OK" if n >= min_people else "Toplu Taşıma - güzergah/saat 4 kişi altı")
    df["departure_note"] = df["departure_service_count"].apply(lambda n: "Servis OK" if n >= min_people else "Toplu Taşıma - güzergah/saat 4 kişi altı")

    service_rows = []
    for (route, arr_dt), group in df.groupby(["route", "arrival_service_dt"]):
        service_rows.append({
            "type": "Geliş",
            "date": arr_dt.date(),
            "route": route,
            ...
            "status": "Kalkar" if len(group) >= min_people else "Kalkmaz / Toplu Taşıma",
            "people": ", ".join(group["name"].astype(str).tolist()[:10]),
        })
    for (route, dep_dt), group in df.groupby(["route", "departure_service_dt"]):
        service_rows.append({
            "type": "Gidiş",
            "date": dep_dt.date(),
            "route": route,
    ...
    service_summary = pd.DataFrame(service_rows).sort_values(["date", "route", "service_time", "type"]).reset_index(drop=True) if service_rows else pd.DataFrame()
